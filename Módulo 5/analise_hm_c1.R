...
ds_c1 %>% group_by(tratamento) %>% get_summary_stats()
...
ds_c1$tratamento <- factor(ds_c1$tratamento, levels =  c("ctrl",    "cis",     "pan",     "pan+cis", "rom",     "rom+cis"))
...
ds_c1 %>% group_by(tratamento) %>%  identify_outliers(valor)

## Removendo o outlier
clean_ds_c1 <- ds_c1[ -c(32,36,2) , ] # clean_ds_c1 <- ds_c1[ linhas , colunas ] 

clean_ds_c1 %>% group_by(tratamento) %>% identify_outliers(valor)

# =============================================================================
# Teste de Normalidade de Shapiro-Wilk
# =============================================================================  

clean_ds_c1 %>% group_by(tratamento) %>% shapiro_test(valor)
...
clean_ds_c1 %>% levene_test(valor ~ tratamento)
...
clean_ds_c1 %>% kruskal_test(valor ~ tratamento)
clean_ds_c1 %>% kruskal_effsize(valor ~ tratamento, ci = T)

clean_ds_c1 %>% anova_test(valor ~ tratamento, white.adjust = T) # White.adjust = T, eu falo para a fórmula que a variancia é diferente
...
clean_ds_c1 %>% anova_test(valor ~ tratamento, effect.size = 'pes')
